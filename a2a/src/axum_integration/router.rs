...
use crate::server::dispatch::{DispatchResult, dispatch};
...
async fn handle_rpc<H: A2AHandler + 'static>(
	...
	let is_streaming = crate::server::dispatch::is_streaming_method(&request.method);
...
fn build_request_context(headers: &axum::http::HeaderMap) -> RequestContext {
	use crate::server::context::{ApiKey, BearerToken, CorrelationId};
		...
		&& value[..7].eq_ignore_ascii_case("bearer ")
	{
		context
			.extensions_mut()
			.insert(BearerToken(value[7..].to_owned()));
	}
...
fn serialise_or_error(value: &impl serde::Serialize) -> String {
	serde_json::to_string(value).unwrap_or_else(|error| {
		serde_json::to_string(&A2AError::internal_error(format!(
			"serialisation failed: {error}"
		)))
		.unwrap_or_else(|_| {
...
async fn forward_stream_events(
	events: crate::server::handler::EventStream<'_>,
...
mod tests {
	...
	use crate::agent_card::{AgentCapabilities, AgentCard, AgentCardRequired};
	use crate::error;
	use crate::jsonrpc::JsonRpcResponse;
	use crate::message::Message;
	use crate::operation::{SendMessageParams, SendMessageResult};
	use crate::role::Role;
	use crate::server::context::RequestContext;
	...
	impl A2AHandler for EchoHandler {
		...
		) -> impl std::future::Future<Output = Result<SendMessageResult, crate::error::A2AError>>
		+ Send
		+ '_ {
			async move {
	...
	fn test_agent_card() -> AgentCard {
		use crate::agent_card::AgentInterface;
	...
	mod loopback {
		use super::*;
		use crate::client::discovery::discover_agent;
		use crate::client::rpc::A2AClient;
		use crate::operation::SendMessageParams;
		...
		async fn loopback_discover_and_send_message() {
			...
			// TCP.
			let params = SendMessageParams::new(crate::message::Message::text(
				"loopback-msg-1",
			...
			match result {
				crate::operation::SendMessageResult::Message(msg) => {
					assert_eq!(
						msg.role,
						crate::role::Role::Agent,
				...
				crate::operation::SendMessageResult::Task(_) => {
		...
		async fn loopback_unknown_method_returns_error() {
			...
			let rpc: crate::jsonrpc::JsonRpcResponse = resp
				.json()
				.await
		...
		async fn loopback_streaming_message_returns_sse_events() {
			use crate::client::sse::into_event_stream;
			use futures_util::StreamExt;
			...
			match &events[0] {
				crate::stream_event::StreamResponse::StatusUpdate(update) => {
					assert_eq!(update.task_id, "sse-task-1");
					assert_eq!(update.status.state, crate::task_state::TaskState::Working);
				}
			...
			match &events[1] {
				crate::stream_event::StreamResponse::StatusUpdate(update) => {
					assert_eq!(update.task_id, "sse-task-1");
					assert_eq!(update.status.state, crate::task_state::TaskState::Completed);
				}
	...
	impl A2AHandler for StreamingHandler {
		fn message_stream(
			...
			Output = Result<crate::server::handler::EventStream<'_>, crate::error::A2AError>,
		> + Send
		+ '_ {
			async move {
				...
				let events: Vec<
					Result<crate::stream_event::StreamResponse, crate::error::A2AError>,
				> = vec![
					Ok(crate::stream_event::StreamResponse::StatusUpdate(
						crate::stream_event::TaskStatusUpdateEvent::new(
							"sse-task-1",
							context_id.clone(),
							crate::task::TaskStatus::working(),
						),
					)),
					Ok(crate::stream_event::StreamResponse::StatusUpdate(
						crate::stream_event::TaskStatusUpdateEvent::new(
				...
				Ok(Box::pin(futures_util::stream::iter(events))
					as crate::server::handler::EventStream<'_>)
			}
		}
	}
	...
	impl A2AHandler for AuthCapturingHandler {
		...
		+ Send
		+ '_ {
			use crate::server::context::BearerToken;
