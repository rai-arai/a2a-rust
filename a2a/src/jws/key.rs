...
fn parse_rsa_jwk(
	...
	let modulus_uint = rsa::BigUint::from_bytes_be(&n_bytes);
	let exponent_uint = rsa::BigUint::from_bytes_be(&e_bytes);

	let public_key = rsa::RsaPublicKey::new(modulus_uint, exponent_uint)
		.map_err(|error| VerificationError::InvalidJwk(format!("invalid RSA key: {error}")))?;
