...
//! Server module—handler trait and JSON-RPC dispatch for A2A agents.
//!
//! This module provides the core abstractions for building A2A agents:
//!
//!   handler —the `A2AHandler` trait that agents implement
...
//!
//! Gated behind the "server" feature flag.
...
mod tests {
	...
	use crate::operation;
	use crate::operation::discovery::GetExtendedAgentCardParams;
	use crate::operation::push_notification::{
	...
	use crate::operation::task_management::{
	...
	use crate::stream_event::{StreamResponse, TaskStatusUpdateEvent};
	...
	impl A2AHandler for TaskHandler {
		...
		fn message_stream(
			...
			_context: &RequestContext,
			params: SendMessageParams,
		) -> impl std::future::Future<Output = Result<EventStream<'_>, crate::error::A2AError>> + Send + '_
		{
			async move {
	...
	impl A2AHandler for PushNotificationHandler {
		fn set_push_notification_config(
			...
			Output = Result<TaskPushNotificationConfig, crate::error::A2AError>,
		> + Send
		+ '_ {
			async move {
		...
		fn list_push_notification_configs(
			...
			Output = Result<ListTaskPushNotificationConfigsResponse, crate::error::A2AError>,
		> + Send
		+ '_ {
			async move {
		...
		) -> impl std::future::Future<Output = Result<(), crate::error::A2AError>> + Send + '_ {
			async move {
	...
	impl A2AHandler for ExtendedCardHandler {
		...
		) -> impl std::future::Future<Output = Result<AgentCard, crate::error::A2AError>> + Send + '_
		{
			async move {
	...
	impl A2AHandler for ListTasksHandler {
		...
		) -> impl std::future::Future<Output = Result<ListTasksResponse, crate::error::A2AError>>
		+ Send
		+ '_ {
			async move {
				...
				let task_a = crate::task::Task::new(
					"listed-task-a",
					context.clone(),
					crate::task::TaskStatus::submitted()
						.with_timestamp(datetime!(2026-03-24 08:00:00 UTC)),
				);
				let task_b = crate::task::Task::new(
					"listed-task-b",
					context,
	...
	impl A2AHandler for ContextCapturingHandler {
		...
		+ '_ {
			...
			async move {
				let reply = crate::message::Message::text("ctx-reply", Role::Agent, token);
				Ok(SendMessageResult::Message(reply))
			}
		}
	}
