...
use crate::handler::{A2AHandler, EventStream};
	...
	HandlerFuture: std::future::Future<Output = std::result::Result<Result, A2AError>>,
{
	...
	let typed_params: Params = match serde_json::from_value(params_value) {
		Ok(parsed) => parsed,
		Err(error) => {
			// Sanitise the serde error for the wire—include the field path
			// and category (missing field, invalid type) but not Rust type
			// names or internal structure details.
			return DispatchResult::Response(JsonRpcResponse::error(
				A2AError::invalid_params(sanitise_serde_error(&error)),
				id.clone(),
			));
		}
	};
	...
	match handler_fn(typed_params).await {
		Ok(result) => match serde_json::to_value(result) {
			...
			// Serialisation of a handler result should never fail—the types
			// derive Serialize. If it does, send a generic error without
			// exposing internal details on the wire.
			Err(_error) => DispatchResult::Response(JsonRpcResponse::error(
				A2AError::internal_error("failed to serialise response"),
				id.clone(),
			)),
		},
...
/// Extract a caller-safe description from a serde deserialization error.
///
/// Serde errors can contain Rust type names and internal structure paths
/// that shouldn't reach the wire. This function extracts the useful part
/// (which field is missing or has the wrong type) while stripping
/// implementation details like "expected struct Foo".
fn sanitise_serde_error(error: &serde_json::Error) -> String {
	let message = error.to_string();

	// serde_json errors follow patterns like:
	//   "missing field `foo` at line 1 column 42"
	//   "invalid type: string \"bar\", expected u32 at line 1 column 5"
	// Strip everything after ", expected" to remove Rust type names.
	if let Some(position) = message.find(", expected") {
